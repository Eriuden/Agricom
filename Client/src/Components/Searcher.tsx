...
export const Searcher = () => {
    ...
    const handleSearchChange = (e: any) => {
        ...
        || article.productType.toLowerCase().includes(search.toLowerCase()))
        setSearch(result)
    }
  return (
    <div className='flex justify-center'>
      <form action='' className='border-2 rounded-sm my-4 max-w-[100%] border-black'>
        <input className='text-center' type="text" placeholder="Article/type"
      ...
      <div>
        { !isEmpty(articles[0]) && search !="" ? (
          articles.map((article:any) => {
            if (article.name.includes(search) || article.productType.includes(search)){
